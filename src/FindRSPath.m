...
function [isok, t, u, v] = LpRpLmRm(x, y, phi)
    xi = x + sin(phi);
    eta = y - 1 - cos(phi);
...
% formula 8.8
function [isok, t, u, v] = LpRmLmRp(x, y, phi)
    xi = x + sin(phi);
    eta = y - 1 - cos(phi);
...
function [isok, path] = CCCC(x, y, phi)
    ...
    [isok, t, u, v] = LpRpLmRm( - x, y,  - phi); % timeflip
    if isok
        L = abs(t) + 2 * abs(u) + abs(v);
        if Lmin > L
            Lmin = L;
    ...
    [isok, t, u, v] = LpRpLmRm(x, -y, -phi); % reflect
    if isok
        L = abs(t) + 2 * abs(u) + abs(v);
        if Lmin > L
            Lmin = L;
    ...
    [isok, t, u, v] = LpRpLmRm(-x, -y, phi); % timeflip  +  reflect
    if isok
        L = abs(t) + 2 * abs(u) + abs(v);
        if Lmin > L
            Lmin = L;
    ...
    [isok, t, u, v] = LpRmLmRp(-x, y, -phi); % timeflip
    if isok
        L = abs(t) + 2 * abs(u) + abs(v);
        if Lmin > L
            Lmin = L;
    ...
    [isok, t, u, v] = LpRmLmRp(x, -y, -phi); % reflect
    if isok
        L = abs(t) + 2 * abs(u) + abs(v);
        if Lmin > L
            Lmin = L;
    ...
    [isok, t, u, v] = LpRmLmRp(-x, -y, phi); % timeflip  +  reflect
    if isok
        L = abs(t) + 2 * abs(u) + abs(v);
        if Lmin > L
            Lmin = L;
...
% formula 8.11
function [isok, t, u, v] = LpRmSmLmRp(x, y, phi)
    xi = x + sin(phi);
    eta = y - 1 - cos(phi);
    [~, rho] = cart2pol(xi, eta);
    if rho >= 2
...
function [isok, path] = CCSCC(x, y, phi)
    ...
    [isok, t, u, v] = LpRmSmLmRp(x, y, phi); % timeflip
    if isok
        L = abs(t) + abs(u) + abs(v);
        if Lmin > L
            Lmin = L;
    ...
    [isok, t, u, v] = LpRmSmLmRp(x, y, phi); % reflect
    if isok
        L = abs(t) + abs(u) + abs(v);
        if Lmin > L
            Lmin = L;
    ...
    [isok, t, u, v] = LpRmSmLmRp(x, y, phi); % timeflip  +  reflect
    if isok
        L = abs(t) + abs(u) + abs(v);
        if Lmin > L
            Lmin = L;
...
function PlotPath(path, veh)
    ...
    axis equal
    plot(0, 0, 'kx', 'LineWidth', 2, 'MarkerSize', 5)
    plot(x(end), y(end), 'ko',  'LineWidth', 2, 'MarkerSize', 5)
    allPath = plot(x(1), y(1), 'd', 'MarkerFaceColor', 'g', 'MarkerSize', 5);
    videoFWriter = VideoWriter('all_path.mp4', 'MPEG-4');
    open(videoFWriter)
